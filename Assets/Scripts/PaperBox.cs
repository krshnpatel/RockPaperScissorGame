...
public class PaperBox : MonoBehaviour
{
	...
	void OnCollisionEnter(Collision col)
	{
		...
		if (col.gameObject.name == "PaperBox_Enemy") // Tie case
		{
			Animator anim = col.gameObject.GetComponent<Animator> (); // Get the animator from the object colliding
			PlayAnimation(anim, "PaperBox_EnemyKill", "Tie", col.gameObject);
			Score.total++; // Increment the total games played
		}
		else if (col.gameObject.name == "RockBox_Enemy") // Win case
		{
			Animator anim = col.gameObject.GetComponent<Animator> (); // Get the animator from the object colliding
			PlayAnimation(anim, "RockBox_EnemyKill", "Win", col.gameObject);
			Score.wins++; // Increment number of games won
			Score.total++; // Increment the total games played
		}
		else if (col.gameObject.name == "ScissorBox_Enemy") // Lose case
		{
			Animator anim = this.gameObject.GetComponent<Animator> (); // Get the animator from the object colliding
			PlayAnimation(anim, "PaperBoxKill", "Lost");
	...
	void PlayAnimation (Animator anim, string animName, string result, GameObject go = null)
	{
		...
		{
			this.gameObject.GetComponent<Animator>().CrossFade ("PaperBoxKill", 0f);
